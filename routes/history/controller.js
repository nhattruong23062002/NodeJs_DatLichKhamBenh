...
module.exports = {
  getAll: async (req, res, next) => {
    try {
      let results = await db.History.findAll()
  ...
  getDetail: async (req, res, next) => {
    try {
      const { id } = req.params;
      let found = await db.History.findOne({
        where: {id: id}
      });
  ...
  create: async function (req, res, next) {
    try {
      ...
      const newItem = new db.History(data);
  ...
  update: async function (req, res, next) {
    try {
      ...
      const found = await db.History.update( updateData, {
        where: {id: id}
      });
      if (found) {
        return res.send({
  ...
  remove: async function (req, res, next) {
    try {
      ...
      let found  = await db.History.destroy({
        where: { id: id },
      });
